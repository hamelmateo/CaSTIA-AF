...
def run_umap_on_cells(
    ...
    """
    traces = [cell.trace.versions["smoothed"] for cell in active_cells
          if len(cell.trace.versions["smoothed"]) > 0 and not cell.exclude_from_umap]
...
def run_umap_with_clustering(
    ...
    """
    traces = [cell.trace.versions["smoothed"] for cell in active_cells if len(cell.trace.versions["smoothed"]) > 0]
